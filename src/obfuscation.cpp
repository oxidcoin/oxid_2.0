...
void CObfuscationPool::ProcessMessageObfuscation(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsa") { //Obfuscation Accept Into Pool
        ...
        if (sessionUsers == 0) {
            if (pmn->nLastDsq != 0 &&
                pmn->nLastDsq + mnodeman.CountEnabled(CMasternode::nodeTier::UNKNOWN, ActiveProtocol()) / 5 > mnodeman.nDsqCount) {
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Oxid and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 OXID per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::IsCollateralValid(const CTransaction& txCollateral)
{
...

    {
        ...
        if (!AcceptableInputs(mempool, state, txCollateral, true, NULL)) {
            LogPrintf("CObfuscationPool::IsCollateralValid() - didn't pass AcceptableInputs\n");
            return false;
        }
    }
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeOxidAmount * COIN - pwalletMain->GetAnonymizedBalance();
    ...
    if (!sessionFoundMasternode) {
        ...
        int nThreshold = (int)(mnodeman.CountEnabled(CMasternode::nodeTier::UNKNOWN, ActiveProtocol()) * 0.9);
        ...
        while (i < 10) {
            CMasternode* pmn = mnodeman.FindRandomNotInVec(CMasternode::nodeTier::UNKNOWN, vecMasternodesUsed, ActiveProtocol());
            if (pmn == NULL) {
...
void CObfuscationPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100OXID+1 ( bit on if present )
    // bit 1 - 10OXID+1
    // bit 2 - 1OXID+1
    // bit 3 - .1OXID+1
    // bit 3 - non-denom
...
bool CObfuScationSigner::IsVinAssociatedWithPubkey(CTxIn& vin, CPubKey& pubkey)
{
    ...
    BOOST_FOREACH (CTxOut out, txVin.vout) {
        if(!CMasternode::HasValidCollateral(out.nValue))
            continue;
