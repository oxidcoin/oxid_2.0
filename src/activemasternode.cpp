...
bool CActiveMasternode::GetMasterNodeVin(CTxIn& vin, CPubKey& pubkey, CKey& secretKey, std::string strTxHash, std::string strOutputIndex)
{
    ...
    if (!strTxHash.empty()) {
        ...
        BOOST_FOREACH (COutput& out, possibleCoins) {
            ...
            LogPrintf("CActiveMasternode::GetMasterNodeVin() nValue: %d\n", out.tx->vout[out.i].nValue);

            if (out.tx->GetHash() == txHash && out.i == outputIndex) {

                if(!CMasternode::mnTier(out.tx->vout[out.i].nValue)) {
                    continue;
                }
        ...
        if (possibleCoins.size() > 0) {
            ...
            auto selectedTier = CMasternode::mnTier(selectedOutput->tx->vout[selectedOutput->i].nValue);
            ...
            for(auto& out : possibleCoins) {
                ...
                if(CMasternode::mnTier(out.tx->vout[out.i].nValue) > selectedTier)
                    selectedOutput = &out;
            }
        } else {
