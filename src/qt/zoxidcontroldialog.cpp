...
#include "ui_zoxidcontroldialog.h"
...
std::list<std::string> ZOxidControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZOxidControlDialog::listMints;

ZOxidControlDialog::ZOxidControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZOxidControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZOxidControlDialog::~ZOxidControlDialog()
{
    delete ui;
}

void ZOxidControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZOxidControlDialog::updateList()
{
...
void ZOxidControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZOxidControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZOxid_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZOxidControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZOxidControlDialog::GetSelectedMints()
{
...
void ZOxidControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
