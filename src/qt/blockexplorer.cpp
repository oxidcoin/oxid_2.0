...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::OXID, nValue);
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", false)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (oxid.conf).");
            QMessageBox::warning(this, "Oxid Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
...
{
    QString CSS = "body {font-size:12px; color:#333333; bgcolor:#d1d1d1;}\n a, span.addr {color:#EE3F4B; font-weight: bold;}\n table tr td {padding: 3px; border: 1px solid black; background-color: #d1d1d1;}\n td.d0 {font-weight: bold; color:#333333;}\n h2, h3 { white-space:nowrap; color:#d1d1d1;}\n a { color:#515f68; text-decoration:none; }\n a.nav {color:#545454;}\n";
