                                                    ...
                                                    fFeeMinimized(true)
{
    ...
    if (!settings.contains("bUseInstantTX"))
        settings.setValue("bUseInstantTX", false);

    bool useInstantTX = settings.value("bUseInstantTX").toBool();
    if (fLiteMode) {
        ui->checkInstantTX->setVisible(false);
        CoinControlDialog::coinControl->useObfuScation = false;
        CoinControlDialog::coinControl->useInstantTX = false;
    } else {
        ui->checkInstantTX->setChecked(useInstantTX);
        CoinControlDialog::coinControl->useInstantTX = useInstantTX;
    }

    connect(ui->checkInstantTX, SIGNAL(stateChanged(int)), this, SLOT(updateInstantTX()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (ui->checkInstantTX->isChecked()) {
        recipients[0].useInstantTX = true;
        strFunds += " ";
        strFunds += tr("and InstantTX");
    } else {
        recipients[0].useInstantTX = false;
    }
...
void SendCoinsDialog::updateInstantTX()
{
    QSettings settings;
    settings.setValue("bUseInstantTX", ui->checkInstantTX->isChecked());
    CoinControlDialog::coinControl->useInstantTX = ui->checkInstantTX->isChecked();
    coinControlUpdateLabels();
}
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Oxid address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
