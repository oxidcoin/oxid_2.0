...
{
    std::string blockType = fProofOfStake ? "Proof-of-Stake" : "Proof-of-Work";
    CBlockIndex* pindexPrevious = chainActive.Tip();
    int previousBlocknHeight = 0;
    if (pindexPrevious) {
        previousBlocknHeight = pindexPrevious->nHeight;
    }
    std::string netType = Params().NetworkID() == 0 ? "MainNet" : "TestNet";
    LogPrintf("+++++++++++++++++++++++++++++++++  Creating new %s block on %s. Block nHeight=%d  ++++++++++++++++++++++++++++++++++++++\n", blockType, netType, previousBlocknHeight + 1);
    ...
    pblock->vtx.push_back(txNew);            // 1st TRANSACTION
    pblocktemplate->vTxFees.push_back(-1);   // updated at end
    pblocktemplate->vTxSigOps.push_back(-1); // updated at end

    // oxid: if coinstake available add coinstake tx
    static int64_t nLastCoinStakeSearchTime = GetAdjustedTime(); // only initialized at startup
    LogPrintf("miner.cpp CreateNewBlock() nLastCoinStakeSearchTime=%d\n", nLastCoinStakeSearchTime);
    if (fProofOfStake) {
        ...
        if (nSearchTime >= nLastCoinStakeSearchTime) {
            ...
            if (pwallet->CreateCoinStake(*pwallet, pblock->nBits, nSearchTime - nLastCoinStakeSearchTime, txCoinStake, nTxNewTime)) {
                // TX2<EMPTY-vout, vout.nCredit-masternodePayment, MN-vout.payee.nValue>
                pblock->nTime = nTxNewTime;
                pblock->vtx[0].vout[0].SetEmpty(); // ALREADY EMPTY
                pblock->vtx.push_back(CTransaction(txCoinStake)); // 2nd TRANSACTION
                fStakeFound = true;
            }
            nLastCoinStakeSearchInterval = nSearchTime - nLastCoinStakeSearchTime;
            nLastCoinStakeSearchTime = nSearchTime;
        }
        LogPrintf("miner.cpp CreateNewBlock() 146: fStakeFound=%d\n", fStakeFound);
        if (!fStakeFound)
            return NULL;
    }
    ...
    CAmount nFees = 0;
    {
        ...
        while (!vecPriority.empty()) { // ADD ALL TRANSACTIONS TO THE BLOCK
        ...
        uint256 nCheckpoint;
        int aHeight = nHeight - (nHeight % 10) - 10;
        if (aHeight <= 0) {
            aHeight = 0;
        } else {
            uint256 hashBlockLastAccumulated = chainActive[aHeight]->GetBlockHash();
        ...
        LogPrintf("miner.cpp -> CreateNewBlock() Block before validating: %s\n", pblock->ToString());
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("OxidMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("OxidMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("OxidMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("oxid-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running Oxid Miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        LogPrintf("ThreadBitcoinMiner started\n");
