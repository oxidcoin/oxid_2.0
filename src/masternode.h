...
//
// The Masternode Class. For managing the Obfuscation process. It contains the input of the 5000 or 25000 OXID, signature to prove
...
public:
    ...
    std::string getSupernodeStatusString() {
        std::string supernodeStatus = "MN";
    ...
    static std::string mnTierToString(unsigned mnTier) {
    ...
    unsigned mnTier()
    {
        return mnTier(deposit);
    }

    static bool HasValidCollateral(CAmount vinValue);
    static bool HasValidCollateral(const CTxIn& vin, CAmount& vinValue);
    static unsigned mnTier(CAmount vinValue);
    static unsigned mnTier(const CTxIn& vin);
