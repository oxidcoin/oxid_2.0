...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "oxid.conf"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 28932, 28942));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in OXID/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in OXID/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-zmqpubhashtxlock=<address>", _("Enable publish hash transaction (locked via InstantTX) in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawblock=<address>", _("Enable publish raw block in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtx=<address>", _("Enable publish raw transaction in <address>"));
    strUsage += HelpMessageOpt("-zmqpubrawtxlock=<address>", _("Enable publish raw transaction (locked via InstantTX) in <address>"));
#endif
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in OXID/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-instanttxdepth=<n>", strprintf(_("Show N confirmations for a successfully locked transaction (0-9999, default: %u)"), nInstantTXDepth));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 28933, 48885));
...
std::string LicenseInfo()
{
    return FormatParagraph(_("Copyright (C) 2009-2014 The Bitcoin Core Developers")) + "\n" +
           ...
           "\n" +
           FormatParagraph(_("Copyright (C) 2016-2017 The Pivx Core Developers")) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The Oxid Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Oxid is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!GetBoolArg("-enableinstanttx", fEnableInstantTX)) {
        if (SoftSetArg("-instanttxdepth", 0))
            LogPrintf("AppInit2 : parameter interaction: -enableinstanttx=false -> setting -nInstantTXDepth=0\n");
    }
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Oxid is shutting down."));
...
#endif
    // Make sure only a single Oxid process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Oxid is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    if (!fDisableWallet) {
        ...
        if (GetBoolArg("-resync", false)) {
            ...
            filesystem::path accumulatorDir = GetDataDir() / "accumulators";
            ...
            try {
                ...
                if (filesystem::exists(accumulatorDir)){
                    boost::filesystem::remove_all(accumulatorDir);
                    LogPrintf("-resync: folder deleted: %s\n", accumulatorDir.string().c_str());
                }
            } catch (boost::filesystem::filesystem_error& error) {
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Oxid: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Oxid") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Oxid to complete") << "\n";
        ...
        bool fEnableZOxidBackups = GetBoolArg("-backupzoxid", true);
        pwalletMain->setZOxidAutoBackups(fEnableZOxidBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeOxidAmount = GetArg("-anonymizeoxidamount", 0);
//    if (nAnonymizeOxidAmount > 999999) nAnonymizeOxidAmount = 999999;
//    if (nAnonymizeOxidAmount < 2) nAnonymizeOxidAmount = 2;

    fEnableInstantTX = GetBoolArg("-enableinstanttx", fEnableInstantTX);
    nInstantTXDepth = GetArg("-instanttxdepth", nInstantTXDepth);
    nInstantTXDepth = std::min(std::max(nInstantTXDepth, 0), 60);
    ...
    /* Denominations
       ...
       For example:
       1OXID+1000 == (.1OXID+100)*10
       10OXID+10000 == (1OXID+1000)*10
    */
