...
Value listmasternodes(const Array& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1))
        throw runtime_error(
            ...
            "    \"addr\": \"addr\",    (string) Masternode Oxid address\n"
...
Value masternodeconnect(const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 1))
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("masternodeconnect", "\"192.168.0.6:28932\"") + HelpExampleRpc("masternodeconnect", "\"192.168.0.6:28932\""));
...
Value getmasternodecount (const Array& params, bool fHelp)
{
    if (fHelp || (params.size() > 0))
        throw runtime_error(
            ...
            "  \"masternode_total\": n,        (numeric) Total masternodes\n"
            "  \"masternode_stable\": n,       (numeric) Stable masternode count\n"
            "  \"masternode_enabled\": n,      (numeric) Enabled masternodes\n"
            "  \"masternode_inqueue\": n       (numeric) Masternodes in queue\n"
            "  \"supernode_total\": n,        (numeric) Total supernodes\n"
            "  \"supernode_stable\": n,       (numeric) Stable supernode count\n"
            "  \"supernode_enabled\": n,      (numeric) Enabled supernode\n"
            "  \"supernode_inqueue\": n       (numeric) Supernode in queue\n"
            "  \"total\": n,        (numeric) Total masternodes and supernodes\n"
            "  \"stable\": n,       (numeric) Total Stable masternode and supernode count\n"
            "  \"enabled\": n,      (numeric) Total Enabled masternodes and supernodes\n"
            "  \"inqueue\": n       (numeric) Total Masternodes and Supernodes in queue\n"
            "}\n"
            "\nExamples:\n" +
    ...
    if (chainActive.Tip()) {
        mnodeman.GetNextMasternodeInQueueForPayment(chainActive.Tip()->nHeight, mnTier, true, mn_nCount);
    }

    int mn_total = mnodeman.size(mnTier);
    int mn_stable = mnodeman.stable_size(mnTier);
    int mn_enabled = mnodeman.CountEnabled(mnTier);

    obj.push_back(Pair("masternode_total", mn_total));
    obj.push_back(Pair("masternode_stable", mn_stable));
    obj.push_back(Pair("masternode_enabled", mn_enabled));
    ...
    if (chainActive.Tip()) {
        mnodeman.GetNextMasternodeInQueueForPayment(chainActive.Tip()->nHeight, snTier, true, sn_nCount);
    }

    int sn_total = mnodeman.size(snTier);
    int sn_stable = mnodeman.stable_size(snTier);
    int sn_enabled = mnodeman.CountEnabled(snTier);

    obj.push_back(Pair("supernode_total", sn_total));
    obj.push_back(Pair("supernode_stable", sn_stable));
    obj.push_back(Pair("supernode_enabled", sn_enabled));

    // Total
    obj.push_back(Pair("total", mn_total + sn_total));
    obj.push_back(Pair("stable", mn_stable + sn_stable));
    obj.push_back(Pair("enabled", mn_enabled + sn_enabled));
    obj.push_back(Pair("inqueue", mn_nCount + sn_nCount));
...
Value getmasternodestatus (const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) Oxid address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
