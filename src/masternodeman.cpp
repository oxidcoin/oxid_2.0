...
CValidationState CMasternodeMan::CheckCollateralInTx(const CTxIn& vin, CMutableTransaction& tx)
{
    CValidationState state;
    CAmount mnCollateral;

    if(!CMasternode::HasValidCollateral(vin, mnCollateral)) {
        LogPrintf("CMasternodeMan::CheckCollateralInTx(): Checking masternode collateral: invalid vin amount. Collateral: %d\n", mnCollateral);
        state.Invalid(false, 0, "Checking masternode collateral: invalid vin amount");
        return state;
    }
    ...
    innerTx.vin.push_back(vin);
    innerTx.vout.push_back(vout);
    tx = innerTx;
...
int CMasternodeMan::stable_size(unsigned mnTier)
{
    int nStable_size = 0;
...
int CMasternodeMan::CountEnabled(unsigned mnTier, int protocolVersion)
{
    int i = 0;
...
std::map<unsigned, int> CMasternodeMan::CountEnabledByTiers(int protocolVersion)
{
    if(protocolVersion == -1)
        protocolVersion = masternodePayments.GetMinMasternodePaymentsProto();

    std::map<unsigned, int> result;
    result.emplace(CMasternode::nodeTier::MASTERNODE, 0);
    result.emplace(CMasternode::nodeTier::SUPERNODE, 0);
...
CMasternode* CMasternodeMan::FindRandomNotInVec(unsigned mnTier, std::vector<CTxIn>& vecToExclude, int protocolVersion)
{
    LOCK(cs);

    protocolVersion = protocolVersion == -1 ? masternodePayments.GetMinMasternodePaymentsProto() : protocolVersion;

    int nCountEnabled = CountEnabled(mnTier, protocolVersion);
    ...
    BOOST_FOREACH (CMasternode& mn, vMasternodes) {
        if (mn.protocolVersion < protocolVersion || !mn.IsEnabled()) continue;
        if (mnTier != CMasternode::nodeTier::UNKNOWN && mn.mnTier() != mnTier) continue;
