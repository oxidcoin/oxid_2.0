...
CMasternode::CMasternode(const CMasternodeBroadcast& mnb)
{
    ...
    if(HasValidCollateral(mnb.vin, deposit))
        activeState = MASTERNODE_ENABLED;
    else
    {
        deposit = 0;
...
uint256 CMasternode::CalculateScore(int mod, int64_t nBlockHeight)
{
    ...
    // LogPrintf("CMasternode::CalculateScore() mod=%d nBlockHeight=%d\n", mod, nBlockHeight);
    uint256 hash = 0;
...
void CMasternode::Check(bool forceCheck)
{
    ...
    if (!unitTest) {
        ...
        CValidationState state = CMasternodeMan::CheckCollateralInTx(vin, tx);
...

        {
            ...
            if (!AcceptableInputs(mempool, state, CTransaction(tx), false, NULL)) {
                activeState = MASTERNODE_VIN_SPENT;
                LogPrintf("CMasternode::Check(): didn't pass AcceptableInputs: %s\n", tx.ToString());
                return;
            }
        }
    }
...
int64_t CMasternode::GetLastPaid()
{
    ...
    int nMnCount = mnodeman.CountEnabled(mnTier()) * 1.25;
    int n = 0;
    for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
        if (n >= nMnCount) {
            return 0;
...
unsigned CMasternode::mnTier(CAmount vinValue)
{
    switch(vinValue) {
        ...
        case collateral::SUPERNODE_COLLATERAL  * COIN:
            return nodeTier::SUPERNODE;
    }
...
bool CMasternode::HasValidCollateral(CAmount vinValue)
{
    return mnTier(vinValue);
}

bool CMasternode::HasValidCollateral(const CTxIn& vin, CAmount& vinValue)
{
    ...
    CAmount vinAmount = prevoutTx.vout[vin.prevout.n].nValue;

    if(!HasValidCollateral(vinAmount)) {
        return false;
    }
...
std::string CMasternode::ToString()
{
    string sTier;
    if(mnTier() == nodeTier::MASTERNODE) {
        sTier = "MASTERNODE";
    } else if (mnTier() == nodeTier::SUPERNODE) {
        sTier = "SUPERNODE";
    } else {
        sTier = "UNKNOWN";
    }
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
...

    {
        ...
        if (!AcceptableInputs(mempool, state, CTransaction(tx), false, NULL)) {
            //set nDos
            LogPrintf("CMasternodeBroadcast::CheckInputsAndAdd(): didn't pass AcceptableInputs: %s\n", tx.ToString());
            state.IsInvalid(nDoS);
            return false;
        }
    }
    ...
    // should be at least not earlier than block when 5000 OXID tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = 0;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 5000 OXID tx -> 1 confirmation
