...
{
    //Oxid will hash in the transaction hash and the index number in order to make sure each hash is unique
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    COxidStake* oxidInput = new COxidStake();
    oxidInput->SetInput(txPrev, txin.prevout.n);
    stake = std::unique_ptr<CStakeInput>(oxidInput);
